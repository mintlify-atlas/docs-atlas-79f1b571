...
New code should use `setWatchlistMembership`, `setProgressStatus`, and `setReaction` instead.
</Note>
...
<Note>
This sets `inWatchlist` to false but keeps the watch item record. 
New code should use `setWatchlistMembership` with `inWatchlist: false` instead.
</Note>
...
<Warning>
This is a migration utility for existing data. It processes the user's watch items 
and maps legacy status values to the new progressStatus/reaction model.
</Warning>